...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length) {
      ...
      //  .setTitle("Usage")
        .setDescription(
          `<:Zayn_salah:841625444594876417>・${message.client.prefix}playlist **YouTube Playlist URL** | **Playlist Name**`
        );
    ...
    if (!permissions.has("CONNECT")) {
      const nullVC = new MessageEmbed()
        .setColor(message.guild.me.displayHexColor)
      //  .setTitle("Error!")
        .setDescription("<:Zayn_salah:841625444594876417>・Cannot connect to voice channel, missing permissions");
    ...
    if (!permissions.has("SPEAK")) {
      ...
      //  .setTitle("Voice Channel Error")
        .setDescription(
          "<:Zayn_salah:841625444594876417>・I cannot speak in this voice channel, make sure I have the proper permissions"
        );
    ...
    if (serverQueue && channel !== message.guild.me.voice.channel) {
      const sameVcErr = new MessageEmbed()
        ...
        //.setTitle("Error!")
        .setDescription(
          `<:Zayn_salah:841625444594876417>・You must be in the same channel as ${message.client.user}`
        );
    ...
    if (spotifyPlaylistValid) {
      try {
        waitMessage = await message.channel.send(
          new MessageEmbed()
            .setDescription("<a:Zayn_loading:841236694376120341> ・Fetching Playlist...")
            .setColor(message.guild.me.displayHexColor)
      ...
      try {
        ...
        const playlistNotFound = new MessageEmbed()
          .setColor(message.guild.me.displayHexColor)
          //.setTitle("Not Found")
          .setDescription("<:Zayn_salah:841625444594876417>・Playlist Not Found");
        return message.reply(playlistNotFound);
      }
    } else if (scdl.isValidUrl(args[0])) {
      if (args[0].includes("/sets/")) {
        message.channel.send("<a:Zayn_loading:841236694376120341>・Fetching Playlist...");
    ...
    if (playlistEmbed.description.length >= 2048)
      playlistEmbed.description =
        ...
        "\n<:Zayn_salah:841625444594876417>・Playlist larger than character limit...";

    let bentar = await message.channel.send("<a:Zayn_centang:841289776379461702>・Succes Added Playlist!").then(message => message.delete({ timeout:2000 }))
